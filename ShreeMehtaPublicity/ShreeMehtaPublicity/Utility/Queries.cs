...
namespace ShreeMehtaPublicity.Utility
{
    public class Queries
    {
        ...
        public string getAllSites = "SELECT SITE_SEQ, SITE_NAME, SITE_STATUS, SITE_ADDRESS, SITE_HEIGHT, SITE_WIDTH, SITE_AMOUNT FROM SITE_MASTER";
        ...
        public string getSiteList = "SELECT SITE_SEQ, SITE_NAME, SITE_STATUS, SITE_ADDRESS, SITE_HEIGHT, SITE_WIDTH, SITE_AMOUNT FROM SITE_MASTER WHERE (SITE_STATUS = '?' OR '?' = 'ALL') AND (SITE_HEIGHT = '?' OR '?' = 'ALL') AND (SITE_WIDTH = '?' OR '?' = 'ALL') AND (SITE_AMOUNT = '?' OR '?' = 'ALL') AND (lower(SITE_NAME) like '%?%' OR '?' = 'ALL') AND (lower(SITE_ADDRESS) like '%?%' OR '?' = 'ALL') AND SITE_SEQ NOT IN (SELECT SITE_SEQ_NO FROM ORDER_MASTER WHERE STATUS IN ('RUNN','PREE') AND ((date('?') BETWEEN date(START_DATE) AND date(END_DATE)) OR (date('?') BETWEEN date(START_DATE) AND date(END_DATE)) OR (date(START_DATE) BETWEEN date('?') AND date('?')) OR (date(END_DATE) BETWEEN date('?') AND date('?'))))";
        public string addNewSite = "INSERT INTO SITE_MASTER (SITE_SEQ, SITE_NAME, SITE_ADDRESS, SITE_HEIGHT, SITE_WIDTH, SITE_AMOUNT, SITE_STATUS) VALUES (?,'?','?','?','?','?','ACTV')";
        public string mdfySite = "UPDATE SITE_MASTER SET SITE_NAME = '?', SITE_ADDRESS = '?', SITE_HEIGHT = '?', SITE_WIDTH = '?', SITE_AMOUNT = '?' WHERE SITE_SEQ = ?";
